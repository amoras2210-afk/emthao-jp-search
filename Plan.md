...
- ✅ If one scraper fails, the others still show results.
...
| File | Purpose |
|---|---|
| `package.json` | Node 20, deps: express, express-rate-limit, playwright, lru-cache, p-limit (v3 CJS), pino, pino-pretty, cors, dotenv |
| `Dockerfile` | `mcr.microsoft.com/playwright:v1.59.1-jammy` base (pinned to match `playwright` in package.json), `node src/server.js` |
...
| `src/scrapers/paypay.js` | Lightweight HTTP warmup (fallback to a full page load on failure) mints the session cookie; DOM scrape with geo-block detector; 404 treated as genuine zero-result, 403/5xx/data-fetch-failed retried |
| `src/paypaySession.js` | In-memory cache of PayPay's 4 essential session cookies (30-min TTL) — lets subsequent searches in the same process skip the warmup entirely |
| `src/routes/search.js` | `GET /search?q=&sources=&yahooMode=&limit=&page=&nocache=` — cache + parallel scrapers behind a process-wide concurrency limiter (`SCRAPE_CONCURRENCY`, shared across all requests) + per-source retry (3 attempts, [1s, 2s] backoff) with a derived, capped deadline (`MAX_SEARCH_DEADLINE_MS`) + per-IP rate limit (20 req/min on this route) + dedup + pagination |
...
| FR-10 | Retry logic | ✅ (frontend 3-attempt + backend per-scraper retry, 3 attempts with [1s, 2s] backoff) |
...
### Common ops
- **PayPay** now works from non-JP IPs via an HTTP warmup + session-cookie cache (`src/scrapers/paypay.js`, `src/paypaySession.js`) — validated locally (VPN-simulated non-JP IP); see `.claude/CLAUDE.md`. Not yet re-verified against a live Render Singapore deployment.
- **Concurrency and rate limiting are process-wide**: the scraper concurrency limiter (`SCRAPE_CONCURRENCY`, default 2) and the per-IP rate limiter (20 req/min on `/search`) are both a single shared instance for the whole process, not created per-request — protects the free-tier memory budget when multiple users search at once.
...
- **Phase 2 (~$5/mo):** Upstash Redis cache (replace `lru-cache`); promote client filters to backend (`price_min/max/condition`); selector breakage canary.
...
- **Phase 5 (~$15/mo):** Webshare residential JP proxy (no longer required for PayPay — see `.claude/CLAUDE.md`); Redis-backed rate limiting for multi-instance scaling (a basic in-memory per-IP limit on `/search`, 20 req/min, is already in place for the current single-instance deployment); Grafana Cloud dashboard.
